...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (req.files && req.files.length > 0) {
        ...
        for (const file of req.files) {
          try {
            ...
            console.log(
              `Document processed and uploaded: ${file.originalname}`
            );
          } catch (uploadError) {
      ...
      await digitalizeLandDocuments(savedLand._id);
...
router.get("/my-lands", auth, async (req, res) => {
  try {
    const lands = await Land.findByOwner(req.user._id).sort({ createdAt: -1 });
...
// Update digitalizeLandDocuments to expect _id
async function digitalizeLandDocuments(landId) {
  if (!landId) {
